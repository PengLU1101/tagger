...
class Fc_Layer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        out = self.fc(inputs.contiguous().view(batch_size*seq_len, -1))
        if self.activation:
            out = F.relu(out)
...
class Classifier(nn.Module):
    ...
    def forward(self,
                ...
                real_tag,
                mask):
        batchsize, seq_len, d_in = logits.size()
        logits = logits * mask.unsqueeze(-1)
        if self.crf_flag:
            return self.crf(logits) * mask.unsqueeze(-1)
             
        else:
            return self.logsm(logits)    
