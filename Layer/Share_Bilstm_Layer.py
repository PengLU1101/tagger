...
class Share_lstm_cell(nn.Module):
    def __init__(self, d_in, d_hid, bias=True, layernorm=False, share=2):
        ...
        self.weight_hh = nn.Parameter(torch.Tensor(self.num_gate * d_hid, d_hid))
        if bias:
            self.bias_ih = nn.Parameter(torch.Tensor(self.num_gate * d_hid))
            self.bias_hh = nn.Parameter(torch.Tensor(self.num_gate *d_hid))
        else:
    ...
    def reset_parameters(self):
        stdv = 1.0 / math.sqrt(self.d_hid)
        #stdv = np.sqrt(3.0 / self.d_hid)
        for weight in self.parameters():
            #if weight.dim == 2:
                #stdv = np.sqrt(6.0 / (weight.size(0) / self.num_gate + weight.size(1)))
    ...
    def forward(self, input, h_minors, c_minors, mask):
        """
        ...
        ifob, c_hat = torch.split(tmp+tmp2, (self.num_gate-1)*self.d_hid, dim=1)
        i, f, o, *b = torch.split(F.sigmoid(ifob), self.d_hid, dim=1)
        c_t = f * c_minors + i * F.tanh(c_hat)
        c_t = c_t * mask
        h = o * F.tanh(c_t) * mask
        #sub = self.share_num - 1 - torch.sum(torch.stack(b, dim=0), dim=0)
        #s = [(2.0 * bb - sub) * F.tanh(c_t) * mask for bb in b]
        s = [bb * F.tanh(c_t) * mask for bb in b]
        #s = [(bb + (1 - o)) / 2 * F.tanh(c_t) * mask for bb in b]
...
class StackedLSTMCell(nn.Module):
    ...
    def forward(self, input, h_0, c_0, mask):
        h_1, c_1, s_1 = [], [], []
        input = self.Dropout(input)
        for i, layer in enumerate(self.layers):
            h_1_i, c_1_i, s_1_i = layer(input, h_0[i], c_0[i], mask)
            input = h_1_i
...
class BiSLSTM(nn.Module):
    ...
    def forward(self, inputs, mask):
        ...
        for i in range(seq_len):
            _, h, c, s_list = self.RNN(inputs[i], h, c, mask[i])
            h_list += [h[-1]]
            c_list += [c[-1]]
            s_lists += [[s[-1] for s in s_list]]
            #s_lists += [s_list]
            _, h_r, c_r, s_r_list = self.RNN_r(inputs[seq_len-i-1], h_r, c_r, mask[seq_len-i-1])
            h_r_list += [h_r[-1]]
            c_r_list += [c_r[-1]]
