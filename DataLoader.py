...
def multitask_dataloader(path, num_task, batch_size=8):
    ...
    for data_name, lists in data.items():
        ...
        dev_loadaer = get_loader(lists['devMatrix'], mappings, data_name, batch_size)
        test_loadaer = get_loader(lists['testMatrix'], mappings, data_name, batch_size)
...
def collate_fn(data):
    ...
    src_seqs, tgt_seqs, src_feats, src_chars = zip(*data)
    ...
    return src_seqs, src_masks.float(), src_feats, tgt_seqs, tgt_masks.float(), src_chars, tgt_list
