...
class Neural_Tagger_cnn(nn.Module):
    def __init__(self, Word_embeddings, Feature_embeddings, Char_embeddings,
        ShareRNN, CNN, Classifierlist, concat_flag):
    ...
    def forward(self, src_seqs, src_masks, src_feats, src_chars,
        ...
        tgt_seqs = tgt_seqs
        tgt_masks = tgt_masks
        feats, h_p, h_s = self.encode(src_words, src_masks, src_feats, src_chars, taskidx)
        loss = self.Classifierlist[taskidx](feats, tgt_seqs, src_masks)
        return loss, h_p, h_s

    def encode(self, src_words, src_masks, src_feats, src_chars, taskidx):
        ...
        inputs = torch.cat((word_embeds, feat_embeds, char_cnn_out), dim=-1)
    ...
    def predict(self, src_seqs, src_masks, src_feats, src_chars,
        ...
        tgt_seqs = tgt_seqs
        tgt_masks = tgt_masks
        feats, _, _ = self.encode(src_words, src_masks, src_feats, src_chars, taskidx)
...
def build_model_cnn(para):
    ...
    ShareRNN = BiSLSTM(rnn)
    CNN = Char_CNN(d_char_emb, d_char, kernel_size, padding, dropout=.5)
    if concat_flag:
        d_in_fc = d_hid * 2
    else:
        d_in_fc = d_hid
