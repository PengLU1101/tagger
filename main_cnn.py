...
def main():
    ...
    model = Model_crf.build_model_cnn(para)
    ...
    def lr_decay(optimizer, epoch, decay_rate=0.9, init_lr=0.015):
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
        return optimizer
    def exp_lr_decay(optimizer, epoch, decay_rate=0.15, init_lr=0.015):
    ...
    if args.optim == "noam":
        ...
        model_optim = optim.Adam(params, lr=0.0015, betas=(0.9, 0.98), eps=1e-9, weight_decay=1e-8)
    if args.mode == "train":
        best_F1 = 0
        if not para["crf"]:
        ...
        for epoch_idx in range(NUM_EPOCH):
            ...
            use_time = time.time() - start_point
                ...
                #if loss_list[task2id["conll2003"]] < max(loss_list[task2id["conll2000"]], loss_list[task2id["unidep"]]):
            ...
            if not best_F1 or best_F1 < F1:
                best_F1 = F1
                Model_crf.save_model(model_path, model, para)
                print('*' * 60)
        ...
        model = Model_crf.build_model(para_save)
        model = Model_crf.read_model(model_path, model)
