...
def main():
    ...
    if args.optim == "noam":
        model_optim = optim_custorm.NoamOpt(para["d_hid"], 1, 1000, torch.optim.Adam(params, lr=0.0015, betas=(0.9, 0.98), eps=1e-9, weight_decay=L2))
        args.decay = None
    elif args.optim == "sgd":
        model_optim = optim.SGD(params, lr=0.015, momentum=args.momentum, weight_decay=1e-8)
    elif args.optim == "adam":
        model_optim = optim.Adam(params, lr=0.0, betas=(0.9, 0.98), eps=1e-9, weight_decay=1e-8)
    if args.mode == "train":
        best_F1 = 0
        if not para["crf"]:
...
def run_epoch(model, data_holder, model_optim, calculate_loss, KLLoss, para, epoch_idx, id2task):####
    ...
    print("On test set:")
    show_result(prec_list_test, rec_list_test, f1_list_test, acc_list_test, para["id2task"])
    if args.optim == "noam":
